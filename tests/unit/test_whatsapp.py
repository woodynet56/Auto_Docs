...
async def test_meta_client_sends_document_link(monkeypatch: pytest.MonkeyPatch) -> None:
    FakeAsyncClient.responses = [FakeResponse({"messages": [{"id": "wamid.DOC"}]})]
    monkeypatch.setattr(httpx, "AsyncClient", FakeAsyncClient)
    result = await client().send_document(
        "+525500000001", "https://private.invalid/file", "resultado.pdf", "REQ-ABC"
    )
    assert result == "wamid.DOC"
    assert FakeAsyncClient.payload["type"] == "document"
    assert FakeAsyncClient.payload["to"] == "525500000001"


async def test_meta_client_converts_document_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    FakeAsyncClient.responses = [FakeResponse({}, fail=True)]
    monkeypatch.setattr(httpx, "AsyncClient", FakeAsyncClient)
    with pytest.raises(WhatsAppDeliveryError, match="document"):
