...
class Settings(BaseSettings):
    ...
    OIDC_ADMIN_GROUP: str = "gestoria-admin"
    OIDC_OPERATOR_GROUP: str = "gestoria-operator"
    OIDC_VIEWER_GROUP: str = "gestoria-viewer"
    ADMIN_SESSION_KEY: SecretStr = Field(default=SecretStr(""))
    ADMIN_SESSION_TTL_MINUTES: int = Field(default=60, ge=5, le=480)
    BACKUP_ENCRYPTION_KEY: SecretStr = Field(default=SecretStr(""))
    BACKUP_RETENTION_DAYS: int = Field(default=35, ge=7, le=365)
    RPO_MINUTES: int = Field(default=60, ge=5, le=1440)
    RTO_MINUTES: int = Field(default=120, ge=15, le=1440)
