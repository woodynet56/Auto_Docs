...
class ServiceRequest(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    gestor_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("external_managers.id", ondelete="SET NULL"), nullable=True
    )
    ...
    identifier_encrypted: Mapped[bytes | None] = mapped_column(LargeBinary, nullable=True)
    identifier_hash: Mapped[str | None] = mapped_column(String(64), index=True, nullable=True)
    identifier_masked: Mapped[str] = mapped_column(String(80), default="", server_default="")
    service_type: Mapped[str] = mapped_column(String(80), default="other", server_default="other")
    original_message: Mapped[str] = mapped_column(String(2000), default="", server_default="")
    ...
    client: Mapped["Client"] = relationship(back_populates="requests", foreign_keys=[client_id])  # noqa: F821
    gestor: Mapped["ExternalManager | None"] = relationship(
