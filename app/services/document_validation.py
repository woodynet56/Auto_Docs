...
def validate_document(data: bytes, filename: str | None, maximum_bytes: int) -> ValidatedDocument:
    ...
    elif data.lstrip().startswith(b"<?xml"):
        detected = ValidatedDocument("application/xml", ".xml")
    elif data.startswith(b"PK\x03\x04"):
        suffix = PurePath(filename or "").suffix.lower()
        if suffix == ".docx":
            detected = ValidatedDocument(
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document", ".docx"
            )
        else:
            raise InvalidDocument(
                "ZIP containers are not accepted until archive inspection is enabled"
            )
    if detected is None:
